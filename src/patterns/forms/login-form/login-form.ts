...
export default component(
	...
	(el) => {
		...
		const usernameField: Component<InputFieldProps> | null =
			el.querySelector(".username");
		const passwordField: Component<InputFieldProps> | null =
			el.querySelector(".password");
		if (!usernameField || !passwordField)
			throw new Error("No input fields found");
		return [
			first<LoginFormProps, HTMLFormElement>(
				"form",
			...
			first<LoginFormProps, Component<InputButtonProps>>(
				".login",
				setProperty(
					"disabled",
					() =>
			...
			first<LoginFormProps, HTMLElement>(
				".logout",
