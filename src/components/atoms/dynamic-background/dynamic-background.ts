import {
	type Component, type FxFunction,
	all, component, consume, on, setStyle
} from '@zeix/ui-element'
import 'culori/css'
import { formatCss } from 'culori/fn'
import { getStepColor } from '../../../assets/js/utils'
import { asLCHColor, LCHColor } from '../../../assets/js/parsers'
...
const polyfillShadowRoot = (host: HTMLElement): void => {
	...
	const getTemplate = () => {
		const tpl = (host.getRootNode() as DocumentFragment).getElementById(`${host.localName}-template`)
		return tpl instanceof HTMLTemplateElement ? tpl : null
	}

	let template = host.querySelector<HTMLTemplateElement>('template[shadowrootmode]')
	if (template) {
		host.attachShadow({
			mode: (template.getAttribute('shadowrootmode') ?? 'open') as ShadowRootMode
		}).appendChild(template.content)
		template.remove()
	} else {
		template = getTemplate()
		template ? host.attachShadow({
			mode: 'open'
...
export default component('dynamic-background', {
	...
	const pointerBubble = el.querySelector<HTMLElement>('gradient-bubble[move="pointer"]')
	if (pointerBubble) {
		let posX = 0
		let posY = 0
		let pointerX = 0
		...
		fns.push(on('pointermove', (e: Event) => {
	...
	fns.push(all('gradient-bubble',
		setStyle('--color-bubble',
			(_, i) => i === 4
				? formatCss(el.color)
				: formatCss(getStepColor(el.color, (el.theme === 'dark' ? 1 + i : 9 - i) / 10))
		)
	))
