...
It handles updates to `value` and `description` (hint message) attributes or reactive properties, provides basic form field validation through [ValidityState API](https://developer.mozilla.org/en-US/docs/Web/API/ValidityState), and takes care of neccessary aria-attributes.
...
As with all `UIElement` instances, you can interact with reactive properties through `has(key)`, `get(key)`, `set(key, value)` and `delete(key)` methods on the element and use them in `effect()` methods elsewhere.

Used reactive properties:
