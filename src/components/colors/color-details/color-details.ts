import {
	type Component,
	asString, component, first, FxFunction, RESET, setStyle, setText
} from '@zeix/ui-element'
import 'culori/css'
import { formatCss, formatHex, formatRgb, formatHsl } from 'culori/fn'
...
export default component('color-details', {
	...
	for (const [name, fn] of Object.entries({
		...
		hue: () => `${formatNumber(el.color.h)}°`,
		oklch: () => `oklch(${formatNumber(el.color.l, 4)} ${formatNumber(el.color.c, 4)} ${formatNumber(el.color.h)})`,
